...
abstract class GitHubClientBase
{
    ...
    public function getLastPage()
    {
        if (!isset($this->pageData['last']))
                throw new GitHubClientException("Last page not defined",
            GitHubClientException::PAGE_INVALID);
    ...
    public function getNextPage()
    {
        ...
        if (is_null($this->page))
                throw new GitHubClientException("Page not defined",
            GitHubClientException::PAGE_INVALID);
    ...
    protected function doRequest($url, $method, $data, $contentType = null,
            $filePath = null)
    {
        if ($method == 'FILE') $url = $this->uploadUrl . $url;
    ...
    {
        return $this->request($this->lastUrl, $this->lastMethod, $data,
                        $this->lastExpectedHttpCode, $this->lastReturnType,
                        $this->lastReturnIsArray);
    }

    public function request($url, $method, $data, $expectedHttpCode,
            $returnType, $isArray = false)
    {
        ...
        if (is_array($data) && !is_null($this->page))
        {
            ...
            {
                $this->resetPage();
                throw new GitHubClientException("Page must be positive value",
                GitHubClientException::PAGE_INVALID);
            }
        ...
        return $this->parseResponse($url, $response, $returnType,
                        $expectedHttpCode, $isArray);
    }

    public function parseResponse($url, $response, $returnType,
            $expectedHttpCode, $isArray = false)
    {
        // parse response
        $header = false;
        $content = array();
        ...
        if ($status !== $expectedHttpCode)
                throw new GitHubClientException("Expected status [$expectedHttpCode], actual status [$status], URL [$url]",
            GitHubClientException::INVALID_HTTP_CODE);

        if ($returnType == 'string') return implode("\n", $content);

        $response = json_decode(implode("\n", $content));

        if ($returnType == 'json') return $response;

        if ($isArray) return GitHubObject::fromArray($response, $returnType);
        else return new $returnType($response);
    }

    public function upload($url, $data, $expectedHttpCode, $returnType,
