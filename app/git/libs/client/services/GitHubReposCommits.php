...
class GitHubReposCommits extends GitHubService
{
     ...
     */
    public function listCommitsOnRepository($owner, $repo, $sha = null,
            $path = null, $author = null, $since = null, $until = null)
    {
        $data = array();
        if (!is_null($sha)) $data['sha'] = $sha;
        if (!is_null($path)) $data['path'] = $path;
        if (!is_null($author)) $data['author'] = $author;
        if (!is_null($since)) $data['since'] = $since;
        if (!is_null($until)) $data['until'] = $until;

        $data['page'] = -1;
        $data['per_page'] = 100;
        
        //$this->client->setPage(3);

        return $this->client->request("/repos/$owner/$repo/commits",
                        'GET', $data, 200, 'json', true);
    }
    ...
    public function getSingleCommit($owner, $repo, $sha)
    {
        ...
        return $this->client->request("/repos/$owner/$repo/commits/$sha", 'GET',
                        $data, 200, 'GitHubFullCommit', true);
    }
    ...
    public function compareTwoCommits($owner, $repo, $base, $head)
    {
        ...
        return $this->client->request("/repos/$owner/$repo/compare/$base...$head",
                        'GET', $data, 200, 'GitHubCommitComparison');
    }
